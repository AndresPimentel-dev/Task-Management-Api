...
def register_user(user_data: UserCreate, db: Session = Depends(get_db)): #noqa: B008
    ...
    username = db.query(UsersTable).filter(UsersTable.username == user_data.username).first()
...
def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)): #noqa: B008
    user = db.query(UsersTable).filter(UsersTable.username == form_data.username).first()
